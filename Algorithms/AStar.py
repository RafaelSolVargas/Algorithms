    ...
    def search_path(self, start_position: tuple, end_position: tuple, diagonal=False) -> list:
        ...
        while len(self.__open_list) > 0:
            ...
            if self.__check_finish(current_node, end_position):
                path = self.__get_traceback(start_node, current_node)
                closed_points = [node.position for node in self.__closed_nodes]

                return path, closed_points
...
if __name__ == '__main__':
    ...
    def test_pathfinder():
        ...
        print(f'Points opened and not used: {len(abertos) - len(path)}')
